...
const FormattedAIResponse = ({ text }) => {
  ...
  const parseResponse = (response = "") => {
    ...
    if (typeof response !== "string") return sections;

    // Split by section headers
    const explanationMatch = response.match(
      /EXPLANATION:([\s\S]*?)(?=EXAMPLE:|SUMMARY:|$)/i,
    );
  ...
  const hasStructuredResponse =
    sections.explanation || sections.example || sections.summary;
    ...
    <div className="space-y-4">
      ...
      {sections.summary && (
        <div className="bg-slate-700/30 rounded-lg p-4 border border-fuchsia-400/20">
          ...
          <div className="space-y-2 pl-3">{formatText(sections.summary)}</div>
        </div>
      )}
    </div>
  );
